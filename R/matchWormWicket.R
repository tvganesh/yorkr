...
# Date :  6 Nov 2021
# Function: matchWormWicketGraph
# This function computes  and plots the match worm wicket chart
#
###########################################################################################
#' @title
...
matchWormWicketGraph <- function(match,t1,t2,plot=1) {
  ...
  b <- select(a,ball,totalRuns,wicketPlayerOut)
  ...
  b1 <- select(a,ball,totalRuns,wicketPlayerOut)
  # Check for both possibilities
  ...
  e= filter(d,wicketPlayerOut !=  "nobody")
  e1= filter(d1,wicketPlayerOut !=  "nobody")
  ...
  if(plot ==1){ #ggplot2
    ggplot() +
      ...
      geom_point(data=e, aes(x=ball, y=total,color=t1),shape=0) +
